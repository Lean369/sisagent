...
from ..tools.tools_n8n import invoke_n8n
from ..tools.tools_tienda_nube import consultar_orden_tiendanube, consultar_productos_tiendanube
from ..tools.tools_calendar import completar_auth_calendar, agendar_cita_calendar, consultar_citas_calendar
from ..services.analytics import registrar_evento
...
def _lanzar_metricas_background(response_msg, thread_id, latency_ms, isLlmPrimary=True):
    """Lanza el registro de métricas en un hilo independiente para no bloquear."""
    hilo = threading.Thread(
        target=registrar_evento,
        args=(response_msg, thread_id, latency_ms, isLlmPrimary),
...
def nodo_chatbot(state: State, config: RunnableConfig):
    ...
    try:
        ...
        _lanzar_metricas_background(response_msg, thread_id, latency_ms, isLlmPrimary=True)
        ...
        try:
            ...
            _lanzar_metricas_background(response_msg, thread_id, latency_ms, isLlmPrimary=False)
...
# ==============================================================================
def procesar_msg_agente_ia(mensaje_usuario: str, config: dict) -> dict:
    try:
...
def transcribir_audio(audio_buffer: bytes, thread_id, audio_format: str = "ogg") -> Optional[str]:
    ...
    try:
        ...
        if TRANSCRIPTION_PROVIDER == "openai":
            ...
            _lanzar_metricas_background(transcription_metrics, thread_id, latency_ms, isLlmPrimary=True)
...
Responde en relación al texto del usuario y estructura la información claramente."""
        ...
        if IMAGE_ANALYSIS_PROVIDER == "openai":
            ...
            base64_limpio = image_base64.strip()
            if "," in base64_limpio:
            ...
            logger.info(f"[IMAGE] Contenido del mensaje para Gemini: {contenido_mensaje}")
...
class ReciboTransferencia(BaseModel):
    monto: str = Field(description="Monto de la transferencia")
    ...
    cuenta_destino: str = Field(description="Cuenta de destino de la transferencia, 22 dígitos para CVU argentino")
    concepto: str = Field(description="Concepto de la transferencia")
    nombre_emisor: str = Field(description="Nombre del emisor de la transferencia")
    nombre_receptor: str = Field(description="Nombre del receptor de la transferencia")


def extract_transfer_receipt_data(image_buffer: bytes, thread_id: str, caption: str = None, mime_type: str = "image/jpeg") -> tuple:
    """
    Extrae datos de un recibo de transferencia (GPT-4 Vision, Gemini, etc.)
    ...
    Returns:
        Tuple[bool, Optional[str]]: Éxito del análisis y descripción/análisis de la imagen o None si hay error
    """
    try: 
        IMAGE_ANALYSIS_ENABLED = os.getenv("IMAGE_ANALYSIS_ENABLED", "true").lower() == "true"
        IMAGE_ANALYSIS_PROVIDER = os.getenv("IMAGE_ANALYSIS_PROVIDER", "openai")  # openai o gemini
...
Responde únicamente con el JSON, sin texto adicional."""
        ...
        if caption:
            prompt_text = f"Si faltan datos en la imagen también considera el texto: '{caption}'.\n\n{prompt_text}"
        ...
        if analysis:
            ...
            # Verificar campos obligatorios en el análisis
            required_fields = ["monto", "cuenta_destino", "fecha", "operacion"]
            missing = []
            if isinstance(parsed, dict):
                for f in required_fields:
                    val = parsed.get(f)
                    if val is None or (isinstance(val, str) and (not val.strip() or val.strip().lower() == "no especificado")):
                        missing.append(f)
            else:
                # No pudimos parsear a JSON: no podemos validar con seguridad
                logger.warning("[IMAGE] No se pudo parsear el análisis a JSON para validar campos obligatorios.")

            if missing:
                logger.warning(f"[IMAGE] Análisis incompleto, faltan campos: {missing}")
                return False,  f"❌ Análisis incompleto, faltan campos: {missing}"

            # Si llegamos acá, los campos obligatorios están presentes (o no parseable pero no vacío)
            logger.info(f"[IMAGE] Análisis exitoso: {str(analysis)[:100].replace(chr(10), ' ')}...")
            return True, parsed if isinstance(parsed, dict) else analysis
        else:
            logger.error("❌ [IMAGE] No se obtuvo análisis")
            return False, "❌ No se obtuvo análisis"
            
    except Exception as e:
        logger.error(f"🔴 [IMAGE] Error analizando imagen: {e}")
        import traceback
        logger.error(traceback.format_exc())
        return False, f"🔴 Error analizando imagen"