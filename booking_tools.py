...
def registrar_lead_en_crm(user_id: str, telefono: str) -> str:
    """
    ...
    """
    logger.info(f"[CRM] Iniciando registro de lead para user_id={user_id}, telefono={telefono}")
    
    # Obtener información del lead
    lead_info = user_lead_info.get(user_id, {})
    logger.debug(f"[CRM] Información del lead: {lead_info}")
    ...
    resultado = crear_lead_krayin(
        ...
        empresa=empresa,
        rubro=rubro,
        volumen_mensajes=volumen_mensajes,
        notas=f"Lead interesado - Solicitó cita de consultoría Goggle Meet"
    )
    
    if resultado['success']:
        # Guardar el lead_id para futuras actualizaciones
        user_lead_info[user_id]['lead_id'] = resultado['lead_id']
        logger.info(f"[CRM] Lead registrado exitosamente - lead_id={resultado['lead_id']}")
        return f"[CRM] {resultado['message']}"
    else:
        logger.error(f"[CRM] Fallo al registrar lead: {resultado.get('error', 'Unknown error')}")
        return f"[CRM] {resultado['message']}"
...
def crear_persona_krayin(nombre: str, telefono: str, email: str = "") -> Optional[int]:
    ...
    try:
        ...
        if response.text.strip().startswith('<!DOCTYPE') or response.text.strip().startswith('<html'):
            logger.error(f"[CRM] ⚠️ La API devolvió HTML en lugar de JSON - Token inválido o expirado")
...
def get_sheets_service():
    ...
    try:
        ...
        logger.error(f"[SHEETS] Archivo {GOOGLE_CREDENTIALS_FILE} no encontrado")
        return None
    except Exception as e:
        logger.error(f"[SHEETS] Error autenticando Google Sheets: {e}")
        return None
